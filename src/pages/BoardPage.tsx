...
import { supabase, type BoardMember, type BoardHistory } from "@/lib/supabase";
...
const BoardPage = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from("board_history").select("*").order("sort_order"),
    ]).then(([{ data: m }, { data: h }]) => {
      if (m) setMembers(m);
      if (h) setHistory(h);
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        {/* Current Board — centered */}
        <section className="py-14 md:py-20">
          <div className="container">
            <h2 className="mb-10 text-center text-2xl font-bold md:text-3xl">Текущ управителен съвет</h2>
            {loading ? (
              <div className="flex justify-center py-12"><div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" /></div>
            ) : (
              <div className="flex flex-wrap justify-center gap-6">
                {members.map(m => (
                  <div key={m.id} className="w-full sm:w-72 rounded-2xl border bg-card overflow-hidden transition-shadow hover:shadow-md">
                    {m.image_url ? (
                    ...
                    <div className="p-5 text-center">
                      ...
                      <div className="space-y-1.5">
                        {m.email && <a href={`mailto:${m.email}`} className="flex items-center justify-center gap-2 text-xs text-muted-foreground hover:text-primary transition-colors"><Mail className="h-3.5 w-3.5" />{m.email}</a>}
                        {m.phone && <a href={`tel:${m.phone}`} className="flex items-center justify-center gap-2 text-xs text-muted-foreground hover:text-primary transition-colors"><Phone className="h-3.5 w-3.5" />{m.phone}</a>}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="mx-auto max-w-3xl">
              ...
              {history.length > 0 && (
                <div className="relative space-y-0">
                  ...
                  {history.map((h) => (
                    <div key={h.id} className="relative flex gap-6 pb-8 last:pb-0">
