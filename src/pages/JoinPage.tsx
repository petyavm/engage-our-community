...
const JoinPage = () => {
  const [membershipFormUrl, setMembershipFormUrl] = useState("http://un.163ou.org/wp-content/uploads/2023/10/zayavlenie-za-priem-v-UN.doc");

  useEffect(() => {
    supabase.from("site_settings").select("value").eq("key", "membership_form_url").single()
      .then(({ data }) => { if (data?.value) setMembershipFormUrl(data.value); });
  }, []);
