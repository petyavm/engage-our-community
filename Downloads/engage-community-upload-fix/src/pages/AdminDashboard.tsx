...
import {
  LogOut, Megaphone, Newspaper, Users, FileText, BarChart2, Image, Heart, Trash2, Plus, Save, X, ChevronDown, ChevronUp, Home, Upload
} from "lucide-react";
      ...
      <div className="rounded-xl border bg-card p-5 space-y-4">
        ...
        {!editId && (
          <div className="rounded-lg border-2 border-dashed border-border p-5 text-center">
            ...
            {uploadProgress && <p className={`mt-2 text-xs ${uploadProgress.startsWith("✓") ? "text-green-600" : "text-muted-foreground"}`}>{uploadProgress}</p>}
          </div>
        )}
      ...
      <div className="space-y-2">
        {items.map(item => (
          <div key={item.id} className="rounded-xl border bg-card p-4 flex items-center gap-3">
            ...
            <div className="flex gap-1 shrink-0">
              <Btn variant="ghost" onClick={() => { edit(item); setUploadProgress(""); }}>Редактирай</Btn>
              <Btn variant="danger" onClick={() => del(item.id)}><Trash2 className="h-4 w-4" /></Btn>
            </div>
          </div>
        ))}
